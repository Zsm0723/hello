...
$graphFormList
	...
	->addRow((new CLabel(_('Graph type'), 'graphtype')),
		(new CComboBox('graphtype', $data['graphtype'], 'submit()', graphType()))->setEnabled(!$readonly)
	)
...
if ($data['graphtype'] == GRAPH_TYPE_NORMAL || $data['graphtype'] == GRAPH_TYPE_STACKED) {
	...
	elseif ($data['ymin_type'] == GRAPH_YAXIS_TYPE_ITEM_VALUE) {
		...
		$ymin_name = '';
		if (!empty($data['ymin_itemid'])) {
			$min_host = get_host_by_itemid($data['ymin_itemid']);

			$minItems = CMacrosResolverHelper::resolveItemNames([get_item_by_itemid($data['ymin_itemid'])]);
	...
	elseif ($data['ymax_type'] == GRAPH_YAXIS_TYPE_ITEM_VALUE) {
		...
		$ymax_name = '';
		if (!empty($data['ymax_itemid'])) {
			$max_host = get_host_by_itemid($data['ymax_itemid']);

			$maxItems = CMacrosResolverHelper::resolveItemNames([get_item_by_itemid($data['ymax_itemid'])]);
...
foreach ($data['items'] as $n => $item) {
	...
	insert_js('loadItem('.$n.', '.json_encode($item['gitemid']).', '.$data['graphid'].', '.$item['itemid'].', '.
